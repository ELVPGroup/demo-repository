...
import { MOCK_PROVIDERS, INITIAL_ORDERS } from '../constants';
import { Order, OrderStatus, LogisticsProvider, DashboardStats } from '../types';
...
const MerchantDashboard: React.FC = () => {
  ...
  const handleAiAnalysis = async () => {
    setIsAnalyzing(true);
    const result = await analyzeDeliveryPerformance(stats, radius, currentProvider);
    setAiInsight(result);
            ...
            {/* <section className="bg-gradient-to-br from-indigo-50 to-purple-50 p-4 rounded-xl border border-indigo-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-xs bg-white hover:bg-indigo-50 border border-indigo-200 text-indigo-600 px-2 py-1 rounded shadow-sm disabled:opacity-50 transition-colors"
                >
                  {isAnalyzing ? '分析中...' : '智能分析'}
                </button>
              </div>
              <p className="text-xs text-indigo-800 leading-relaxed min-h-[40px]">
                {aiInsight || "点击分析获取基于当前配送半径和物流服务商的优化建议。"}
              </p>
            </section> */}
          </div>
